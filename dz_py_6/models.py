...
class Driver(Base):
    """Модель для записей таблицы drivers."""
...
class Client(Base):
    """Модель для записей таблицы clients."""
    ...
    def __repr__(self):
        return (
            "{"
            + f'"id":"{self.id}", "name":"{self.name}", "is_vip":"{self.is_vip}"'
            + "}"
        )


class Order(Base):
    """Модель для записей таблицы orders."""
    ...
    status = Column(
        Enum(Status, validate_strings=True),
        nullable=False,
        default=Status.not_accepted.value,
    )  # not_accepted, in_progress, cancelled, done
    ...
    def __repr__(self):
        return (
            "{"
            + f'"id":"{self.id}","address_from":"{self.address_from}","address_to":"{self.address_to}",'
